...
class Grafo
{
	private:
        ...
        void recorrido(T vertice,std::list<T> &noVisitado, std::list<T> &orden);
        ...
        void CopiarVertice(NodoVertice<T,C> *ptrVer, NodoVertice<T,C> *ptrVer1);
	...
	//Consultores
		...
		int  gradoEntrante(const T &vertice) const;
		int  gradoSaliente(const T &vertice) const;
		int  gradoVertice(const T &vertice)	const;
        bool existeVertice(const T &vertice) const;
        bool existeArco(const T &inicio, const T &fin);
        bool esFuente(const T &vertice)	const;
        bool esSumidero(const T &vertice) const;
        ...
        std::list <T> obtenerSumideros() const;
        std::list<T> predecesores (const T &vertice) const;
        std::list<T> Sucesores(const T &vertice) const;
        std::list<T> getVecinos(const T &vertice) const;
        std::list<T> Fuentes()	const;
...
// Definición de la función 'obtenerSumideros' para la clase 'Grafo'
// La función no toma parámetros y devuelve una lista de elementos de tipo 'T'
template <class T, class C>
std::list <T> Grafo<T,C>::obtenerSumideros() const{
    // Declara una lista de elementos de tipo 'T' llamada 'sumideros'
    std::list<T> sumideros;

    // Declara un puntero a 'NodoVertice' llamado 'verticeAux'
    // y lo inicializa con el valor de 'this->ptrVertice'
    NodoVertice<T,C> *verticeAux=this->ptrVertice;

    // Declara un puntero a 'NodoAdya' llamado 'ady'
    NodoAdya<T,C> *ady;

    // Mientras 'verticeAux' no sea NULL
    while(verticeAux!=NULL){
        // Si el resultado de llamar a la función miembro 'esSumidero' en 'this'
        // con el argumento 'verticeAux->getElemento()' es verdadero quiere decir que el vertice actual es un sumidero
        if(this->esSumidero(verticeAux->getElemento())){
            // Agrega el elemento en 'verticeAux' al final de la lista 'sumideros'
            sumideros.push_back(verticeAux->getElemento());
        }
        // Asigna a 'verticeAux' el resultado de llamar a la función miembro 'getProx' en 'verticeAux' para ir al siguiente nodo
        verticeAux=verticeAux->getProx();
    }

    // Devuelve la lista 'sumideros'
    return(sumideros);
}
...
void Grafo<T,C>::agregarArco(const T &inicio,const T &fin, C costo) {
    ...
    if(this->moverPtrAdya(ptrIni,fin) == NULL) {
        ...
        ptrAdy = new NodoAdya<T,C>(buscarApuntador(fin),ptrIni->getAdya(),costo);
...
template <class T, class C>
bool Grafo<T,C>::esFuente(const T &vertice) const {
    // Declaración de punteros a vértices y adyacencias
    NodoVertice<T,C> *ptrVer;
    NodoAdya<T,C> *ptrAdy;
    // Declaración de variables booleanas
    bool fuente = true, existe = false;
    // Asignación del primer vértice al puntero ptrVer
    ptrVer = ptrVertice;
    // recorrido de la lista de vértices
    while(ptrVer != NULL && fuente) {
        // Si el vértice actual no es el vértice dado y la variable fuente es verdadera
        if(ptrVer->getElemento() != vertice && fuente) {
            // Búsqueda del vértice dado en la lista de adyacencias del vértice actual
            ptrAdy = moverPtrAdya(ptrVer,vertice);
            // Si se encontró el vértice dado en la lista de adyacencias del vértice actual
            if(ptrAdy != NULL) {
                // Asignación de falso a la variable fuente
                fuente = false;
            }
        }else{// Si el vértice actual es el vértice dado
            // Asignación de verdadero a la variable existe
            existe = true;
            // Si el vértice dado no tiene adyacencias
            fuente = ptrVer->getAdya() == NULL;
        }
        ptrVer =  ptrVer->getProx();
    }
    // Si el vértice dado existe en el grafo
    return existe && fuente;

}

template <class T, class C>
bool Grafo<T,C>::esSumidero(const T &vertice) const {
    // Si el vértice no existe en el grafo, devuelve falso
    if(!this->existeVertice(vertice)) {
        return false;
    } else {
        // Declaración de punteros a vértices
        NodoVertice<T,C> *ptrVer, *aux = NULL;
        // Asignación del primer vértice al puntero ptrVer
        ptrVer = this->ptrVertice;
        // Declaración de variable booleana
        bool recibe = false;
        // recorrido de la lista de vértices
        while(ptrVer != NULL && (aux != NULL || !recibe)) {
            // Si el vértice actual es el vértice dado
            if(ptrVer->getElemento() == vertice) {
                aux = ptrVer;
            } else {
                // Si el vértice actual es adyacente al vértice dado
                if(verificarAdya(ptrVer->getElemento(),vertice,ptrVer)) {
                    recibe = true;
                }
            }
            ptrVer = ptrVer->getProx();
        }
        // Si el vértice dado no tiene arcos salientes y recibe arcos entrantes, devuelve verdadero, de lo contrario devuelve falso
        return aux->getAdya() == NULL && recibe;
    }
}
...
// Esta función devuelve una lista de los vecinos de un vértice dado en el grafo
template <class T, class C>
std::list<T> Grafo<T,C>::getVecinos(const T &vertice) const {
    // Declaración de lista de vecinos
    std::list<T> vecinos;
    // Declaración de puntero a vértice
    NodoVertice<T,C> *ptrVer;
    // Asignación del primer vértice al puntero ptrVer
    ptrVer = this->ptrVertice;
    // Declaración de puntero a adyacencia
    NodoAdya<T,C> *ptrAdya;
    // recorrido de la lista de vértices
    while(ptrVer != NULL) {
        // Si el vértice dado es adyacente al vértice actual
        if (moverPtrAdya(ptrVer, vertice) != NULL) {
            // Agrega el vértice actual a la lista de vecinos
            vecinos.push_back(ptrVer->getElemento());
        }
        ptrVer = ptrVer->getProx();
    }

    ptrVer = this->moverVertice(this->ptrVertice,vertice);
    if(ptrVer!=NULL){
        ptrAdya = ptrVer->getAdya();
        while(ptrAdya!=NULL){
            vecinos.push_back(ptrAdya->getVertice()->getElemento());
            ptrAdya = ptrAdya->getProx();
        }
    }
    // Devuelve la lista de vecinosdel vértice dado
    return vecinos;
}

// Esta función devuelve una lista de los predecesores de un vértice dado en el grafo
template <class T, class C>
std::list<T> Grafo<T,C>::predecesores(const T &vertice) const {
    // Declaración de lista de predecesores
    std::list<T> predecessor;
    // Declaración de puntero a vértice
    NodoVertice<T,C> *ptrVer;
    // Asignación del primer vértice al puntero ptrVer
    ptrVer = this->ptrVertice;
    ...
    while(ptrVer != NULL) {
        // Si el vértice dado es adyacente al vértice actual
        if (moverPtrAdya(ptrVer, vertice) != NULL) {
            // Agrega el vértice actual a la lista de predecesores
            predecessor.push_back(ptrVer->getElemento());
        }
        ptrVer = ptrVer->getProx();
    }
    // Devuelve la lista de predecesores del vértice dado
    return predecessor;
}

// Esta función devuelve una lista de los sucesores de un vértice dado en el grafo
template <class T, class C>
std::list<T> Grafo<T,C>::Sucesores(const T &vertice) const {
    ...
    if(ptrVer != NULL) {
        ...
        while(ptrAdy != NULL) {
            ...
            if(!ptrAdy->getVertice()->estaMarcado()) {
        ...
        // recorrido de la lista de adyacencias del vértice dado
        while(ptrAdy != NULL) {
            // Desmarca el vértice adyacente como visitado
            ptrAdy->getVertice()->desmarcar();
            ptrAdy = ptrAdy->getProx();
        }
    }
...
// Esta función devuelve una lista de las fuentes del grafo
template <class T, class C>
std::list<T> Grafo<T,C>::Fuentes() const {
    // Declaración de punteros a vértices y adyacencias
    NodoVertice<T,C> *ptrVer;
    NodoAdya<T,C> *ptrAdy;
    // Declaración de lista de fuentes
    std::list<T> list;
    // Declaramos una lista de vértices marcados para almacenar los vertices que estaban marcados antes de la ejecución de la función
    std::list<T> copiaMarcados = this->getMarcados();
    // Limpiamos todos los vertices
    this->desmarcarTodo();
    // Asignación del primer vértice al puntero ptrVer
    ptrVer = this->ptrVertice;
    // recorrido de la lista de vértices
    while(ptrVer != NULL) {
        // Asignación de la lista de adyacencias del vértice actual al puntero ptrAdy
        ptrAdy = ptrVer->getAdya();
        // recorrido de la lista de adyacencias del vértice actual
        while(ptrAdy != NULL) {
            // Marca el vértice adyacente como visitado
            ptrAdy->getVertice()->marcar();
            ptrAdy = ptrAdy->getProx();
    ...
    // recorrido de la lista de vértices
    while(ptrVer != NULL) {
        // Si el vértice actual no ha sido visitado y tiene adyacencias
        if(!ptrVer->estaMarcado() && ptrVer->getAdya() == NULL) {
            // Agrega el elemento del vértice actual a la lista de fuentes
            list.push_front(ptrVer->getElemento());
        }
        ptrVer = ptrVer->getProx();
    }
    //Marcamos los vertices que estaban marcados antes de la ejecucion de la funcion
    this->marcarVerticesLista(copiaMarcados);
    // Devuelve la lista de fuentes del grafo
    return list;
}
...
template <class T, class C>
int Grafo<T,C>::gradoSaliente(const T &vertice) const
{
    // Declaramos una variable entera para almacenar el grado saliente del vértice
    int grado = 0;
    // Declaramos dos punteros a objetos NodoVertice y NodoAdya
    NodoVertice<T,C> *ptrVer = moverVertice(this->ptrVertice,vertice);
    NodoAdya<T,C> *ptrAdy;
    ...
    if(ptrVer != NULL){
        ...
        while(ptrAdy != NULL)
        {
            // Incrementamos en uno el grado saliente del vértice
            grado++;
            // Avanzamos ptrAdy al siguiente vértice adyacente
            ptrAdy = ptrAdy->getProx();
        }
    ...
    // Devolvemos el valor del grado saliente del vértice
    return grado;
}

template <class T, class C>
int Grafo<T,C>::gradoEntrante(const T &vertice) const
{
    // Declaramos una variable entera para almacenar el grado entrante del vértice
    int grado = 0;
    // Declaramos dos punteros a objetos NodoVertice y NodoAdya
    NodoVertice<T,C> *ptrVer;
    NodoAdya<T,C> *ptrAdy;
    ...
    while(ptrVer != NULL)
    {
        ...
        // Mientras ptrAdy no sea NULL y el elemento de ptrVer no sea igual al valor del vértice dado
        while(ptrAdy != NULL && ptrVer->getElemento() != vertice)
        {
            ...
            if(ptrAdy->getVertice()->getElemento() == vertice){
                // Incrementamos en uno el grado entrante del vértice
                grado++;
            }
            // Avanzamos ptrAdy al siguiente vértice adyacente
            ptrAdy = ptrAdy->getProx();
    ...
    // Devolvemos el valor del grado entrante del vértice
    return grado;
}
